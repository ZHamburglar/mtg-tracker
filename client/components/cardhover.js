...
const CardHover = ({ card }) => {
  ...
  return (
    <HoverCard onOpenChange={handleOpenChange}>
      <HoverCardTrigger>{card.card_name} - ${card.current_price} ({changeSign}{card.percent_change}%)</HoverCardTrigger>
      <HoverCardContent className="w-120 z-50">
        {loading ? (
          <p>Loading card details...</p>
        ) : cardDetails ? (
          <div>
            <p>{card.card_name} is currently priced at ${card.current_price}, which is a {changeSign}{card.percent_change}% change.</p>
